                    ...
                    <div className="container-xxl">
                        ...
                        <div className="h-right d-flex align-items-center mr-5 mr-lg-0 order-1">
                            ...
                            <div className="dropdown">
                                <a aria-label="dropdown" className="nav-link dropdown-toggle pulse" href="#" role="button" data-bs-toggle="dropdown">
                            ...
                            <div className="dropdown companies-list ml-2 ml-sm-3 d-flex align-items-center">
                                <a title='Your Companies' aria-label="companiesList" className="nav-link dropdown-toggle pulse p-0" href="#" role="button" data-bs-toggle="dropdown" data-bs-display="static">
                                ...
                                <div className="dropdown-menu rounded-lg shadow border-0 dropdown-animation dropdown-menu-end p-0 m-0">
                                    <form className="card border-0 w280">
                                       ...
                                       {isLoading ? <Loader/>:<YourCompanies isLoading = {isLoading} companies={companies} activecompany={activecompany} setactivecompany={setactivecompany} />} 
                                    </form>
                                </div>
                            </div>
                            <div className="dropdown user-profile ml-2 ml-sm-3 d-flex align-items-center">
                                <a className="nav-link dropdown-toggle pulse p-0" href="#" role="button" data-bs-toggle="dropdown" data-bs-display="static">
                                    ...
                                    <img className="avata lg rounded-circle Image-thumbnail mx-2" style={{width:"40px", height:"40px"}} src={  accountService.isLogged && loggedUser? api+loggedUser.picture:"/assets/images/profile_av.svg"} alt="profile" quality={60}/>
                                </a>
                                <div className="dropdown-menu rounded-lg shadow border-0 dropdown-animation dropdown-menu-end p-0 m-0">
                                    <div className="card border-0 w280">
                                        <div className="card-body pb-0">
                                            <div className="d-flex py-1">
                                                <img className="avatar rounded-circle" src={accountService.isLogged && loggedUser? api+loggedUser.picture: "/assets/images/profile_av.svg"}   alt="profile"  quality={60}/>
                                                <div className="flex-fill ms-3">
                                                    <p className="mb-0"><span className="font-weight-bold">{loggedUser && loggedUser.firstName} {loggedUser && loggedUser.lastName}</span></p>
                                                    <small className=""> {loggedUser && loggedUser.email} </small>
                                                </div>
                                            </div>
                                        ...
                                        <div className="list-group m-2 ">
                                            ...
                                            <Link onClick={()=>(logout = accountService.logout())} href="" className="list-group-item list-group-item-action border-0">
                                            <i className="bi bi-door-closed-fill text-danger h4"></i>logout
                                            </Link>
                            ...
                            <div className="setting ms-2">
                                <a onClick={()=>(settogglesetting(!togglesetting))} data-bs-toggle="modal" data-bs-target="#Settingmodal">
                        ...
                        {/* <!-- menu toggler --> */}
                        <button role="button" aria-label="menu toggle"  onClick={()=>(settoggleside(!toggleside))} className="navbar-toggler p-0 border-0 menu-toggle order-3" type="button" data-bs-toggle="collapse" data-bs-target="#mainHeader">
...
function YourCompanies({isLoading, companies, activecompany, setactivecompany}) {
    
    const act = (params)=> {
        activecompany.current =(params)
        ...
        <div className="list-group m-2 bg-dak ">
                                            ...
                                            isLoading ? <Loader/>:
                                                companies.length>0 ?companies.map((company, index)=>(
                                                    <label htmlFor={`#id${company.id}-${index}`}  onClick={()=>(alert('company'+JSON.stringify(company)),act(company.id), alert('activecompany'+ JSON.stringify(activecompany)))} key={index} href="" className="list-group-item list-group-item-action border-0 d-flex justify-content-between align-items-center border border-danger border-2 my-3">
                                                    <img className=" lg rounded-circle Image-thumbnail" src={api+ company.logo} alt="profile" quality={60} style={{width:"30px", height:"30px"}} />
                                                 <span className='mx-2'>{company.label.toUpperCase()} {company.label} <strong className="text-danger mx-1">({company.Employees[0].type})</strong> </span>
                                                 <span className='mx-2'> <input type="radio" defaultChecked={company.id === activecompany.current ? true:false} name="activecompany" id={`id${company.id}-${index}`} /> </span>
                                            </label>
                                                )): <span className='p-3 font-weight-bold text-center alert-primary text-primary'>Aucune entreprise disponible</span>
                                            }
